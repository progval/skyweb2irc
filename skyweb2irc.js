...
function decode_skype(text) {
    text = text.replace(/<i raw_pre="_" raw_post="_">/g, '_').replace(/<\/i>/g, '_')
    text = text.replace(/<b raw_pre="\*" raw_post="\*">/g, '*').replace(/<\/b>/g, '*')
    text = text.replace(/<s raw_pre="~" raw_post="~">/g, '~').replace(/<\/s>/g, '~')
